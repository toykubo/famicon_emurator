...
#[allow(dead_code)]
const FLAG_CARRY: u8 = 1; // Set if overflow in bit 7
#[allow(dead_code)]
const FLAG_ZERO: u8 = 1 << 1; // Set if A = 0
#[allow(dead_code)]
const FLAG_INTERRUPT: u8 = 1 << 2; // Not affected
#[allow(dead_code)]
const FLAG_DECIMAL: u8 = 1 << 3; // Not affected
#[allow(dead_code)]
const FLAG_BREAK: u8 = 1 << 4; // Not affected
// 5 は未使用
#[allow(dead_code)]
const FLAG_OVERFLOW: u8 = 1 << 6; // Set if sign bit is incorrect
#[allow(dead_code)]
const FLAG_NEGATIVE: u8 = 1 << 7; // Set if bit 7 set
...
pub struct CPU {
    ...
    // スタックオーバーフローを防ぐためにBox化
    // アドレス空間が固定なら配列型をBoxに入れるのが最も効率的
...
impl CPU {
    pub fn new() -> Self {
        CPU {
            ...
            status: 0,
            program_counter: 0,
            // memory: [0x00; 0x10000], // 0xFFFF
            // vec!を使用してヒープに確保してからBox<[u8; N]>へ変換
    ...
    fn get_operand_address(&self, mode: &AddressingMode) -> u16 {
        match mode {
            ...
            AddressingMode::ZeroPage_X => {
                ...
                pos.wrapping_add(self.register_x) as u16 // addr
            }

            // LDA $4400,X => bd 00 44
            AddressingMode::ZeroPage_Y => {
                let pos = self.mem_read(self.program_counter);
                pos.wrapping_add(self.register_y) as u16 // addr
            }

            // LDA $4400,X => bd 00 44
            AddressingMode::Absolute_X => {
                let base = self.mem_read_u16(self.program_counter);
                base.wrapping_add(self.register_x as u16) // addr
            }

            // LDA $4400,Y => b9 00 44
            AddressingMode::Absolute_Y => {
                let base = self.mem_read_u16(self.program_counter);
                base.wrapping_add(self.register_y as u16) // addr
            }
    ...
    fn mem_read(&self, addr: u16) -> u8 {
        // SAFETY: memory is fixed size 0x10000, addr is u16 so it's always in bounds
        unsafe { *self.memory.get_unchecked(addr as usize) }
    }

    #[inline]
    fn mem_write(&mut self, addr: u16, data: u8) {
        // SAFETY: memory is fixed size 0x10000, addr is u16 so it's always in bounds
        unsafe { *self.memory.get_unchecked_mut(addr as usize) = data };
    }
    ...
    pub fn run(&mut self) {
        ...
        loop {
            ...
            match opscode {
                ...
                0xE8 => self.inx(), // INX - Increment X by one
    ...
    fn sbc(&mut self, mode: &AddressingMode) {
        ...
        let overflow = (self.register_a & SIGN_BIT) != (value & SIGN_BIT)
            && (self.register_a & 0x80) != (n & SIGN_BIT);
...
mod test {
    ...
    #[test]
    fn test_rol_zero_a_page_with_carry() {
        let cpu = run(vec![0x6A, 0x00], |cpu| {
            cpu.register_a = 0x00;
            cpu.status = FLAG_CARRY;
        });
